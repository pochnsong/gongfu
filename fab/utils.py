...
class MySQL(object):
    ...
    @staticmethod
    def create_db(db_name, password, user="root", cmd=None):
        kwargs = {}
        if not cmd:
            cmd = run
            kwargs = {"warn_only": True}

        cmd('echo "CREATE DATABASE IF NOT EXISTS %s DEFAULT CHARSET utf8 COLLATE utf8_general_ci;"|mysql -u%s -p%s'
            % (db_name, user, password),
            **kwargs)

    @staticmethod
    def create_user(new_user, new_password, password, user="root", cmd=None):
        kwargs = {}
        ...
        cmd('echo "CREATE USER \'%s\'@\'localhost\' IDENTIFIED BY \'%s\';" | mysql -u%s -p%s'
            % (new_user, new_password, user, password), **kwargs)

    @staticmethod
    def grant_user(db_name, grant_user, password, user="root", cmd=None):
        kwargs = {}
        ...
        cmd('echo "GRANT ALL ON %s.* TO \'%s\'@\'localhost\';" | mysql -u%s -p%s'
            % (db_name, grant_user, user, password), **kwargs)


def init_mysql_db(db_name, db_pwd, db_user):
    """initialize MySQL database"""
    run('echo "CREATE DATABASE %s;" | mysql -uroot -p%s' % (db_name, db_pwd), warn_only=True)
    run('echo "CREATE USER \%s\'@\'localhost\' IDENTIFIED BY \'%s\';" | mysql -uroot -p%s'
        % (db_user, db_pwd, db_pwd), warn_only=True)
    run('echo "GRANT ALL ON %s.* TO \'%s\'@\'localhost\';" | mysql -uroot -p%s'
        % (db_name, db_user, db_pwd), warn_only=True)
